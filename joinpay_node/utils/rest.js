...
/*
	...
	Use:	var rest = require('./rest');
			rest.init({quiet: false});						//set default values here for all calls
	...
	timeout: Integer in milliseconds to time out requests. Defaults 15000
...
function http(options, parameters, body){
	...
	options = copyAtoB(options, default_options);
		...
		resp.on('end', function() {																	//wait for end before decision
			if(resp.statusCode == 204){																//empty response, don't parse body
				...
				if(acceptJson){
					...
					if(goodJSON){
						if(!options.quiet) console.log(http_txt + ' Data:');						//all good [json resp]
	...
	request.setTimeout(Number(options.timeout) || 20000);
...
module.exports.init = function(opt){
	for(var i in opt){
		if(default_options[i]) default_options[i] = opt[i];
	}
};
